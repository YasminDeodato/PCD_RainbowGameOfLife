...
void geracaoTheards(float **matriz, float **copia, int thread_id, int num_thread, int n) {
  	float passo = ceil((float)n/num_thread);
  	int inicio = thread_id * passo;
	  int fim = (thread_id + 1)* passo - 1; 
...
int interacoes(float **matriz, float nGeracoes, int num_thread, int n) {
        ...
        copiaMatrizes(matriz, copia, n);
            ...
            threads = (pthread_t *)malloc(num_thread * sizeof(pthread_t));
          	...
          	for(i=0; i < num_thread; i++){
          		pthread_create(&threads[i], NULL, geracaoTheards, matriz, copia, i, num_thread, n);
          	}
...
void main() {
    ...
    tempoInicial = clock();
    int resultado = interacoes(matriz, 100, num_thread, n);
