...
int interacoes(float **matriz, float nGeracoes, int n) {
    ...
    for(int g = 0 ; g < nGeracoes; g++){   
        ...
        #pragma omp parallel private(i,j) reduction(+:vivos)
        #pragma omp for
        for(i = 0 ; i <= n-1; i++){   
            for(j = 0; j <= n-1; j++){
                copia[i][j] = verificarNovoEstadoCelula(matriz, n, i, j);
                //matriz[i][j] = copia[i][j];

                if (copia[i][j] > 0.0) vivos++;
            }
        }
...
void main() {
    ...
    tempoInicial = clock();
    int resultado = interacoes(matriz, 2000, n);
